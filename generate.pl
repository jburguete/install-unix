...
if ( $os eq "Linux" ) {
    ...
    if ( $dist eq "Debian" || $dist eq "Devuan" ) {
        ...
        @upgrade = (
            ...
            "system-upgrade", "download", "--release=37;", "dnf",
        ...
        @packages = (
            ...
            "SDL2-devel",                    "texlive-gnu-freefont",
        ...
        @update  = ( "emerge", "--sync" );
        @upgrade = (
            "emerge",         "--ask",    "--update", "--deep",
            "--with-bdeps=y", "--newuse", "\@world"
        );
        @packages = (
...
"PKG_PATH=\"http://cdn.NetBSD.org/pub/pkgsrc/packages/$os/$arch/$ver/All\"\nPATH=\"/usr/pkg/sbin:\$PATH\"\nexport PATH PKG_PATH"
    ...
    if ( $dist eq "5.10.18.git.2a44663-0.1" ) {
        ...
        @clean   = ( "beadm", "list" );
        @update =
          ( "pkg", "update;", "/opt/csw/bin/pkgutil", "-U", "-u", "-y" );
    ...
    @packages = (
        "patch",      "autoconf",      "automake",     "pkgconfig",
        "gcc",        "gcc_fortran",   "make",         "git",
        "subversion", "libxml2_devel", "glib2_devel",  "json_glib_devel",
        "gettext",    "sqlite_devel",  "gsl_devel",    "gtk3_devel",
        "glfw_devel", "libsdl2_devel", "glew_devel",   "vim",
        "indent",     "gdb",           "texlive_core", "graphviz",
        "gnuplot",    "doxygen",       "wget",         "imagemagick",
        "gimp"
    );
}
elsif ( $os eq "Darwin" ) {
...
"/bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)\""
    ...
    @packagescask = (
        ...
        "macvim",  "meld", "mactex-no-gui", "firefox", "thunderbird", "gimp",
        "libreoffice"
    );
    @preinstall = ("@installcask @packagescask");
    @packages   = (
        "gnu-sed",        "gpatch",  "autoconf",    "automake",
        "pkg-config",     "gcc",     "make",        "git",
        "subversion",     "libxml2", "gettext",     "glib",
        "json-glib",      "sqlite",  "gsl",         "libgtop",
        "gtk+3",          "gtk4",    "freeglut",    "glfw",
        "sdl2",           "glew",    "glslang",     "mpich",
        "font-open-sans", "nedit",   "gnu-indent",  "perltidy",
        "maxima",         "ddd",     "graphviz",    "evince",
        "doxygen",        "wget",    "imagemagick", "mpv"
    );
}
else {
    $os = `uname -o`;
